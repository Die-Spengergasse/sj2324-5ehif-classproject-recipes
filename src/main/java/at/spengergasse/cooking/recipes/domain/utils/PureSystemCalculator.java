...
public abstract class PureSystemCalculator {
    ...
    public boolean verify(String input) throws IllegalStateException, IllegalArgumentException {
        ...
        String check = input.substring(dataOnly.length());

        return check.equals(compute(dataOnly));
    }
    ...
    public String compute(String input) throws IllegalStateException {
        ...
        if(doubleDigit) {
            ...
            return Character.toString(chars.charAt(first))+chars.charAt(second);
        } else {
            return Character.toString(chars.charAt(checksum));
        }
    }
